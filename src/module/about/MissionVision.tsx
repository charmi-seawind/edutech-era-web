import * as React from "react";
import Container from "../../components/Container";
import shape from "../../assets/about/shape-5.png";
import about from "../../assets/about/about-2.webp";
import fame from "../../assets/about/fame.png";
import shape2 from "../../assets/about/shape-2.png";
import { MdOutlineContentPaste, MdCastForEducation } from "react-icons/md";
import { FaLanguage } from "react-icons/fa";
import { FaLightbulb } from "react-icons/fa6";
import { GiProgression } from "react-icons/gi";
import { PiCertificateBold } from "react-icons/pi";
import { TbWorldShare } from "react-icons/tb";
import { motion } from "framer-motion"; 
const missionPoints = [
  {
    icon: <MdOutlineContentPaste className="text-blue-600" />,
    text: "Simplify concepts with interactive content",
  },
  {
    icon: <FaLanguage className="text-blue-600" />,
    text: "Offer multilingual support to break language barriers",
  },
  {
    icon: <FaLightbulb className="text-blue-600" />,
    text: "Empower teachers with tools for better learning outcomes",
  },
  {
    icon: <MdCastForEducation className="text-blue-600" />,
    text: "Foster lifelong learning with flexible and relevant educational content.",
  },
];

const visionPoints = [
  {
    icon: <GiProgression className="text-green-600" />,
    text: "Make education interactive, engaging, and personalized",
  },
  {
    icon: <PiCertificateBold className="text-green-600" />,
    text: "Equip educators and students with tools for success.",
  },
  {
    icon: <TbWorldShare className="text-green-600" />,
    text: "Promote critical thinking and problem-solving skills",
  },
  {
    icon: <TbWorldShare className="text-green-600" />,
    text: "Expand access to quality education globally, ensuring every student can thrive.",
  },
];

const MissionVision = () => {
  return (
    <div className="bg-[#f0f8fa] px-4 py-8 md:px-8 md:py-12 relative">
      <Container>
        <div className="absolute -top-10 sm:-top-20 left-4 sm:left-20">
          <img src={shape} alt="Decorative shape" className="w-[80px] sm:w-[150px]" />
        </div>

        {/* Mission Section */}
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="w-full md:w-1/2">
            <img
              src={about}
              alt="About EDUTECH ERA"
              className="w-full max-w-full h-auto rounded object-cover"
            />
          </div>

          <div className="w-full md:w-1/2">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-black mb-4 capitalize">
              Our Mission
            </h2>
            <p className="text-sm sm:text-base">
              We aim to redefine education by leveraging technology to make learning easier, more engaging, and inclusive. Our mission is to:
            </p>

            <div className="my-5">
              {missionPoints.map((point, index) => (
                <motion.div
                  key={`mission-${index}`}
                  initial={{ opacity: 0, x: -30 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  viewport={{ once: true, amount: 0.3 }}
                  className="flex my-3 sm:my-4 gap-3 items-center"
                >
                  <div className="bg-blue-200 p-2 rounded-full">{point.icon}</div>
                  <p className="text-sm sm:text-base">{point.text}</p>
                </motion.div>
              ))}
            </div>
          </div>
        </div>

        {/* Vision Section */}
        <div className="flex flex-col md:flex-row-reverse justify-between items-center gap-8 mt-10">
          <div className="w-full md:w-1/2">
            <img
              src={about}
              alt="Our Vision"
              className="w-full max-w-full h-auto rounded object-cover"
            />
          </div>

          <div className="w-full md:w-1/2">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-black mb-4 capitalize">
              Our Vision
            </h2>
            <p className="text-sm sm:text-base">
              To revolutionize the learning landscape and empower every individual to reach their full potential through technology-driven education.
            </p>

            <div className="my-5">
              {visionPoints.map((point, index) => (
                <motion.div
                  key={`vision-${index}`}
                  initial={{ opacity: 0, x: 30 }}
                  whileInView={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                  viewport={{ once: true, amount: 0.3 }}
                  className="flex my-3 sm:my-4 gap-3 items-center"
                >
                  <div className="bg-green-200 p-2 rounded-full">{point.icon}</div>
                  <p className="text-sm sm:text-base">{point.text}</p>
                </motion.div>
              ))}
            </div>
          </div>
        </div>

        {/* Decorative Images */}
        <div className="absolute bottom-10 left-5 sm:left-10">
          <img src={fame} alt="" className="w-[40px] sm:w-[60px]" />
        </div>
        <div className="absolute right-0 bottom-0">
          <img src={shape2} alt="" className="w-[200px] sm:w-[300px]" />
        </div>
      </Container>
    </div>
  );
};

export default MissionVision;
